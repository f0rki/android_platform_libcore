...
public abstract class MessageDigest extends MessageDigestSpi {
	...
	public static MessageDigest getInstance(String algorithm)
		...
		return new MessageDigestImpl((MessageDigestSpi) sap.spi, sap.provider,
				algorithm);
	}
	...
	public static MessageDigest getInstance(String algorithm, Provider provider)
		...
		return new MessageDigestImpl((MessageDigestSpi) spi, provider,
				algorithm);
	}
	...
	// begin WITH_TAINT_TRACKING
	private void isTainted(int tag) {
		if ((tag & Taint.TAINT_PASSWORD) != 0) {
			Taint.log("Password went into MessageDigest(" + getAlgorithm()
					+ ")");
		}
	}

	private void checkForPasswordTaint(byte input) {
		isTainted(Taint.getTaintByte(input));
	}

	private void checkForPasswordTaint(byte[] input) {
		isTainted(Taint.getTaintByteArray(input));
	}

	private void checkForPasswordTaint(ByteBuffer input) {
		isTainted(input.getDirectByteBufferTaint());
	}
	// end WITH_TAINT_TRACKING
